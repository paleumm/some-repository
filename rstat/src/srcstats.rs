...
pub fn get_stc_stats_for_file(filename: &Path) -> Result<SrcStats, StatsError> {
...
pub fn get_summary_src_stats(dirname: &Path) -> Result<SrcStats, StatsError> {
    let mut total_loc = 0;
    let mut total_comments = 0;
    let mut total_blanks = 0;
    let mut dir_entries: Vec<PathBuf> = vec![dirname.to_path_buf()];
    ...
    for entry in file_entries {
        let stat = get_stc_stats_for_file(&entry.path())?;
        total_loc += stat.loc;
