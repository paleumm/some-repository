...
impl RequestLine {
    ...
    fn get_order_number(&self) -> String {
        let path = self.path();
        let path_token: Vec<String> = path.split("/").map(|s| s.parse().unwrap()).collect();
        path_token[path_token.len() - 1].clone()
    }
}
...
impl FromStr for RequestLine {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut msg_tokens = s.split_ascii_whitespace();
...
fn main() {
    ...
    for stream in connection_listener.incoming() {
        ...
        let req_line = "";
        let string_req_line = if let Some(line) = str::from_utf8(&buffer).unwrap().lines().next() {
            line
        } else {
        ...
        let req_line = RequestLine::from_str(string_req_line).unwrap();

        let html_response_string;
        let order_status;

        print!("len is {}; ", req_line.get_order_number().len());
        ...
        println!("Protocol: {:?}", req_line.protocol);
    }
}
